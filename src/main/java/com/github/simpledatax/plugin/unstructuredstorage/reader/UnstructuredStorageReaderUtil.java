...
/*					CompressionCodec snappyCodec = new SnappyCodec();
	...
	public static void validateCompress(Configuration readerConfiguration) {
		String compress =readerConfiguration
				.getUnnecessaryValue(com.github.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.COMPRESS,null,null);
		...
		readerConfiguration.set(com.github.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.COMPRESS, compress);

	}

	public static void validateFieldDelimiter(Configuration readerConfiguration) {
		//fieldDelimiter check
		String delimiterInStr = readerConfiguration.getString(com.github.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.FIELD_DELIMITER,null);
		if(null == delimiterInStr){
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.REQUIRED_VALUE,
					String.format("您提供配置文件有误，[%s]是必填参数.",
							com.github.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.FIELD_DELIMITER));
	...
	public static void validateColumn(Configuration readerConfiguration) {
		...
		if (null != columns && 1 == columns.size()) {
			...
			if ("\"*\"".equals(columnsInStr) || "'*'".equals(columnsInStr)) {
				readerConfiguration.set(com.github.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.COLUMN, null);
				columns = null;
			}
		}
		...
		if (null != columns && columns.size() != 0) {
			for (Configuration eachColumnConf : columns) {
				eachColumnConf.getNecessaryValue(com.github.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.TYPE,
