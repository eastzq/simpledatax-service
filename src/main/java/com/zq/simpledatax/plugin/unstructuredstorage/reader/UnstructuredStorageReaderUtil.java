package com.zq.simpledatax.plugin.unstructuredstorage.reader;
...
import com.zq.simpledatax.common.element.BoolColumn;
import com.zq.simpledatax.common.element.Column;
import com.zq.simpledatax.common.element.DateColumn;
import com.zq.simpledatax.common.element.DoubleColumn;
import com.zq.simpledatax.common.element.LongColumn;
...
/*					CompressionCodec snappyCodec = new SnappyCodec();
	...
	public static void doReadFromStream(BufferedReader reader, String context,
		...
		List<ColumnEntry> column = UnstructuredStorageReaderUtil
				.getListColumnEntry(readerSliceConfig, UnstructuredReaderKey.COLUMN);
	...
	public static Record transportOneRecord(RecordSender recordSender,
											...
											String line){
		List<ColumnEntry> column = UnstructuredStorageReaderUtil
				.getListColumnEntry(configuration, UnstructuredReaderKey.COLUMN);
		// 注意: nullFormat 没有默认值
	...
	public static void validateEncoding(Configuration readerConfiguration) {
		...
		String encoding = readerConfiguration
				.getString(
						com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.ENCODING,
						com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderConstant.DEFAULT_ENCODING);
		try {
	...
	public static void validateCompress(Configuration readerConfiguration) {
		String compress =readerConfiguration
				.getUnnecessaryValue(com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.COMPRESS,null,null);
		...
		readerConfiguration.set(com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.COMPRESS, compress);

	}

	public static void validateFieldDelimiter(Configuration readerConfiguration) {
		//fieldDelimiter check
		String delimiterInStr = readerConfiguration.getString(com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.FIELD_DELIMITER,null);
		if(null == delimiterInStr){
			throw DataXException.asDataXException(UnstructuredStorageReaderErrorCode.REQUIRED_VALUE,
					String.format("您提供配置文件有误，[%s]是必填参数.",
							com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.FIELD_DELIMITER));
	...
	public static void validateColumn(Configuration readerConfiguration) {
		...
		List<Configuration> columns = readerConfiguration
				.getListConfiguration(com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.COLUMN);
		...
		if (null != columns && 1 == columns.size()) {
			...
			if ("\"*\"".equals(columnsInStr) || "'*'".equals(columnsInStr)) {
				readerConfiguration.set(com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.COLUMN, null);
				columns = null;
			}
		}
		...
		if (null != columns && columns.size() != 0) {
			for (Configuration eachColumnConf : columns) {
				eachColumnConf.getNecessaryValue(com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.TYPE,
						UnstructuredStorageReaderErrorCode.REQUIRED_VALUE);
				Integer columnIndex = eachColumnConf
						.getInt(com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.INDEX);
				String columnValue = eachColumnConf
						.getString(com.zq.simpledatax.plugin.unstructuredstorage.reader.UnstructuredReaderKey.VALUE);
