...
public class SingleTableSplitUtil {
    ...
    public static List<Configuration> splitSingleTable(
            Configuration configuration, int adviceNum,DataBaseType DATABASE_TYPE) {
        List<Configuration> pluginParams = new ArrayList<Configuration>();
        List<String> rangeList;
        String splitPkName = configuration.getString(RdbmsReaderKey.SPLIT_PK);
        String column = configuration.getString(RdbmsReaderKey.COLUMN);
        String table = configuration.getString(RdbmsReaderKey.TABLE);
        String where = configuration.getString(RdbmsReaderKey.WHERE, null);
        ...
        if (DATABASE_TYPE == DataBaseType.Oracle) {
            rangeList = genSplitSqlForOracle(splitPkName, table, where,
                    configuration, adviceNum,DATABASE_TYPE);
            // warn: mysql etc to be added...
        } else {
            Pair<Object, Object> minMaxPK = getPkRange(configuration,DATABASE_TYPE);
            if (null == minMaxPK) {
                throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
                        "根据切分主键切分表失败. DataX 仅支持切分主键为一个,并且类型为整数或者字符串类型. 请尝试使用其他的切分主键或者联系 DBA 进行处理.");
            }

            configuration.set(RdbmsReaderKey.QUERY_SQL, buildQuerySql(column, table, where));
            ...
            boolean isStringType = RdbmsReaderConstant.PK_TYPE_STRING.equals(configuration
                    .getString(RdbmsReaderConstant.PK_TYPE));
            boolean isLongType = RdbmsReaderConstant.PK_TYPE_LONG.equals(configuration
                    .getString(RdbmsReaderConstant.PK_TYPE));
        ...
        if (null != rangeList && !rangeList.isEmpty()) {
            for (String range : rangeList) {
                ...
                tempConfig.set(RdbmsReaderKey.QUERY_SQL, tempQuerySql);
    ...
    public static String buildQuerySql(String column, String table,
        ...
        if (StringUtils.isBlank(where)) {
            querySql = String.format(RdbmsReaderConstant.QUERY_SQL_TEMPLATE_WITHOUT_WHERE,
                    column, table);
        } else {
            querySql = String.format(RdbmsReaderConstant.QUERY_SQL_TEMPLATE, column,
                    table, where);
        }
    ...
    private static Pair<Object, Object> getPkRange(Configuration configuration,DataBaseType DATABASE_TYPE) {
        String pkRangeSQL = genPKRangeSQL(configuration);

        int fetchSize = configuration.getInt(RdbmsReaderConstant.FETCH_SIZE);
        String jdbcURL = configuration.getString(RdbmsReaderKey.JDBC_URL);
        ...
        Pair<Object, Object> minMaxPK = checkSplitPk(conn, pkRangeSQL, fetchSize, table, username, configuration,DATABASE_TYPE);
        DBUtil.closeDBResources(null, null, conn);
        return minMaxPK;
    }

    public static void precheckSplitPk(Connection conn, String pkRangeSQL, int fetchSize,
                                                       String table, String username,DataBaseType DATABASE_TYPE) {
        Pair<Object, Object> minMaxPK = checkSplitPk(conn, pkRangeSQL, fetchSize, table, username, null,DATABASE_TYPE);
        if (null == minMaxPK) {
            throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
                    "根据切分主键切分表失败. DataX 仅支持切分主键为一个,并且类型为整数或者字符串类型. 请尝试使用其他的切分主键或者联系 DBA 进行处理.");
        }
    ...
    private static Pair<Object, Object> checkSplitPk(Connection conn, String pkRangeSQL, int fetchSize,  String table,
                                                     String username, Configuration configuration,DataBaseType DATABASE_TYPE) {
        ...
        try {
            ...
            if (isPKTypeValid(rsMetaData,DATABASE_TYPE)) {
                if (isStringType(rsMetaData.getColumnType(1))) {
                    if(configuration != null) {
                        configuration
                                .set(RdbmsReaderConstant.PK_TYPE, RdbmsReaderConstant.PK_TYPE_STRING);
                    }
                    while (DBUtil.asyncResultSetNext(rs)) {
                        minMaxPK = new ImmutablePair<Object, Object>(
                                rs.getString(1), rs.getString(2));
                    }
                } else if (isLongType(rsMetaData.getColumnType(1),DATABASE_TYPE)) {
                    if(configuration != null) {
                        configuration.set(RdbmsReaderConstant.PK_TYPE, RdbmsReaderConstant.PK_TYPE_LONG);
                    }
    ...
    private static boolean isPKTypeValid(ResultSetMetaData rsMetaData,DataBaseType DATABASE_TYPE) {
        boolean ret = false;
        try {
            int minType = rsMetaData.getColumnType(1);
            int maxType = rsMetaData.getColumnType(2);

            boolean isNumberType = isLongType(minType,DATABASE_TYPE);
    ...
    // Types.NUMERIC
    private static boolean isLongType(int type,DataBaseType DATABASE_TYPE) {
    ...
    private static String genPKRangeSQL(Configuration configuration) {
        ...
        String splitPK = configuration.getString(RdbmsReaderKey.SPLIT_PK).trim();
        String table = configuration.getString(RdbmsReaderKey.TABLE).trim();
        String where = configuration.getString(RdbmsReaderKey.WHERE, null);
    ...
    public static List<String> genSplitSqlForOracle(String splitPK,
            ...
            int adviceNum,DataBaseType DATABASE_TYPE) {
        if (adviceNum < 1) {
        ...
        Double percentage = configuration.getDouble(RdbmsReaderKey.SAMPLE_PERCENTAGE, 0.1);
        ...
        int fetchSize = configuration.getInt(RdbmsReaderConstant.FETCH_SIZE, 32);
        String jdbcURL = configuration.getString(RdbmsReaderKey.JDBC_URL);
        String username = configuration.getString(RdbmsReaderKey.USERNAME);
        String password = configuration.getString(RdbmsReaderKey.PASSWORD);
        ...
        try {
            ...
            if (configuration != null) {
                configuration
                        .set(RdbmsReaderConstant.PK_TYPE, RdbmsReaderConstant.PK_TYPE_MONTECARLO);
            }
            ResultSetMetaData rsMetaData = rs.getMetaData();
            while (DBUtil.asyncResultSetNext(rs)) {
        ...
        if (splitedRangeSize >= 2) {
            ...
            if (isLongType(splitedRange.get(0).getRight(),DATABASE_TYPE)) {
