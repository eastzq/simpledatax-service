package com.zq.simpledatax.plugin.rdbms.util;
...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
    ...
    public static boolean needCheckDeletePrivilege(Configuration originalConfig) {
        ...
        List<String> preSQLs = originalConfig.getList(RdbmsReaderKey.PRE_SQL, String.class);
        List<String> postSQLs = originalConfig.getList(RdbmsReaderKey.POST_SQL, String.class);
    ...
    private static synchronized Connection connect(DataBaseType dataBaseType,
        ...
        //ob10的处理
        if (url.startsWith(com.zq.simpledatax.plugin.rdbms.writer.RdbmsWriterConstant.OB10_SPLIT_STRING) && dataBaseType == DataBaseType.MySql) {
            String[] ss = url.split(com.zq.simpledatax.plugin.rdbms.writer.RdbmsWriterConstant.OB10_SPLIT_STRING_PATTERN);
            if (ss.length != 3) {
    ...
    public static void dealWithSessionConfig(Connection conn,
        ...
        switch (databaseType) {
            case Oracle:
                sessionConfig = config.getList(RdbmsReaderKey.SESSION,
