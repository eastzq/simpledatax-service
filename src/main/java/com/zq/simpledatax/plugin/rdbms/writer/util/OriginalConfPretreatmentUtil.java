package com.zq.simpledatax.plugin.rdbms.writer.util;
...
import com.zq.simpledatax.common.util.ListUtil;
import com.zq.simpledatax.plugin.rdbms.util.ConnectionFactory;
import com.zq.simpledatax.plugin.rdbms.util.DBUtil;
import com.zq.simpledatax.plugin.rdbms.util.DBUtilErrorCode;
import com.zq.simpledatax.plugin.rdbms.util.DataBaseType;
import com.zq.simpledatax.plugin.rdbms.util.JdbcConnectionFactory;
import com.zq.simpledatax.plugin.rdbms.util.TableExpandUtil;
import com.zq.simpledatax.plugin.rdbms.writer.RdbmsWriterConstant;
...
//    }
    ...
    public static void doPretreatment(Configuration originalConfig, DataBaseType dataBaseType) {
        ...
        originalConfig.getNecessaryValue(RdbmsWriterKey.USERNAME, DBUtilErrorCode.REQUIRED_VALUE);
        originalConfig.getNecessaryValue(RdbmsWriterKey.PASSWORD, DBUtilErrorCode.REQUIRED_VALUE);

        doCheckBatchSize(originalConfig,dataBaseType);
    ...
    public static void doCheckBatchSize(Configuration originalConfig,DataBaseType DATABASE_TYPE) {
        // 检查batchSize 配置（选填，如果未填写，则设置为默认值）
        int batchSize = originalConfig.getInt(RdbmsWriterKey.BATCH_SIZE, RdbmsWriterConstant.DEFAULT_BATCH_SIZE);
        if (batchSize < 1) {
        ...
        originalConfig.set(RdbmsWriterKey.BATCH_SIZE, batchSize);
    }

    public static void simplifyConf(Configuration originalConfig,DataBaseType DATABASE_TYPE) {
        List<Object> connections = originalConfig.getList(RdbmsWriterConstant.CONN_MARK,
                Object.class);
        ...
        for (int i = 0, len = connections.size(); i < len; i++) {
            ...
            originalConfig.set(String.format("%s[%d].%s", RdbmsWriterConstant.CONN_MARK, i, RdbmsWriterKey.JDBC_URL),
                    jdbcUrl);

            List<String> tables = connConf.getList(RdbmsWriterKey.TABLE, String.class);
            ...
            originalConfig.set(String.format("%s[%d].%s", RdbmsWriterConstant.CONN_MARK,
                    i, RdbmsWriterKey.TABLE), expandedTables);
        }

        originalConfig.set(RdbmsWriterConstant.TABLE_NUMBER_MARK, tableNum);
    }

    public static void dealColumnConf(Configuration originalConfig, ConnectionFactory connectionFactory, String oneTable,DataBaseType DATABASE_TYPE) {
        List<String> userConfiguredColumns = originalConfig.getList(RdbmsWriterKey.COLUMN, String.class);
        if (null == userConfiguredColumns || userConfiguredColumns.isEmpty()) {
            throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_VALUE,
                    "您的配置文件中的列配置信息有误. 因为您未配置写入数据库表的列名称，DataX获取不到列信息. 请检查您的配置并作出修改.");
        } else {
            boolean isPreCheck = originalConfig.getBool(RdbmsWriterKey.DRYRUN, false);
            ...
            if (1 == userConfiguredColumns.size() && "*".equals(userConfiguredColumns.get(0))) {
                ...
                originalConfig.set(RdbmsWriterKey.COLUMN, allColumns);
    ...
    public static void dealColumnConf(Configuration originalConfig,DataBaseType DATABASE_TYPE) {
        String jdbcUrl = originalConfig.getString(String.format("%s[0].%s",
                RdbmsWriterConstant.CONN_MARK, RdbmsWriterKey.JDBC_URL));
        ...
        String oneTable = originalConfig.getString(String.format(
                "%s[0].%s[0]", RdbmsWriterConstant.CONN_MARK, RdbmsWriterKey.TABLE));

        JdbcConnectionFactory jdbcConnectionFactory = new JdbcConnectionFactory(DATABASE_TYPE, jdbcUrl, username, password);
        dealColumnConf(originalConfig, jdbcConnectionFactory, oneTable,DATABASE_TYPE);
    }

    public static void dealWriteMode(Configuration originalConfig, DataBaseType dataBaseType) {
        List<String> columns = originalConfig.getList(RdbmsWriterKey.COLUMN, String.class);

        String jdbcUrl = originalConfig.getString(String.format("%s[0].%s",
                RdbmsWriterConstant.CONN_MARK, RdbmsWriterKey.JDBC_URL, String.class));

        // 默认为：insert 方式
        String writeMode = originalConfig.getString(RdbmsWriterKey.WRITE_MODE, "INSERT");
        ...
        originalConfig.set(RdbmsWriterConstant.INSERT_OR_REPLACE_TEMPLATE_MARK, writeDataSqlTemplate);
    }

    public static boolean isOB10(String jdbcUrl) {
        //ob10的处理
        if (jdbcUrl.startsWith(com.zq.simpledatax.plugin.rdbms.writer.RdbmsWriterConstant.OB10_SPLIT_STRING)) {
            String[] ss = jdbcUrl.split(com.zq.simpledatax.plugin.rdbms.writer.RdbmsWriterConstant.OB10_SPLIT_STRING_PATTERN);
            if (ss.length != 3) {
