...
public final class WriterUtil {
    ...
    public static List<Configuration> doSplit(Configuration simplifiedConf,
        ...
        int tableNumber = simplifiedConf.getInt(RdbmsWriterConstant.TABLE_NUMBER_MARK);
        ...
        String jdbcUrl;
        List<String> preSqls = simplifiedConf.getList(RdbmsWriterKey.PRE_SQL, String.class);
        List<String> postSqls = simplifiedConf.getList(RdbmsWriterKey.POST_SQL, String.class);

        List<Object> conns = simplifiedConf.getList(RdbmsWriterConstant.CONN_MARK,
                Object.class);
        ...
        for (Object conn : conns) {
            ...
            jdbcUrl = connConf.getString(RdbmsWriterKey.JDBC_URL);
            sliceConfig.set(RdbmsWriterKey.JDBC_URL, jdbcUrl);

            sliceConfig.remove(RdbmsWriterConstant.CONN_MARK);

            List<String> tables = connConf.getList(RdbmsWriterKey.TABLE, String.class);

            for (String table : tables) {
                Configuration tempSlice = sliceConfig.clone();
                tempSlice.set(RdbmsWriterKey.TABLE, table);
                tempSlice.set(RdbmsWriterKey.PRE_SQL, renderPreOrPostSqls(preSqls, table));
                tempSlice.set(RdbmsWriterKey.POST_SQL, renderPreOrPostSqls(postSqls, table));
    ...
    public static List<String> renderPreOrPostSqls(List<String> preOrPostSqls, String tableName) {
        ...
        for (String sql : preOrPostSqls) {
            ...
            if (StringUtils.isNotBlank(sql)) {
                renderedSqls.add(sql.replace(RdbmsWriterConstant.TABLE_NAME_PLACEHOLDER, tableName));
            }
        }
