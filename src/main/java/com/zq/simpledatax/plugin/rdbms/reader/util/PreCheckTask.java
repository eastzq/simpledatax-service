...
public class PreCheckTask implements Callable<Boolean>{
    ...
    public Boolean call() throws DataXException {
        String jdbcUrl = this.connection.getString(RdbmsReaderKey.JDBC_URL);
        List<Object> querySqls = this.connection.getList(RdbmsReaderKey.QUERY_SQL, Object.class);
        List<Object> splitPkSqls = this.connection.getList(RdbmsReaderKey.SPLIT_PK_SQL, Object.class);
        List<Object> tables = this.connection.getList(RdbmsReaderKey.TABLE,Object.class);
            ...
            /*verify splitPK*/
                try{
                    if (splitPkSqls != null && !splitPkSqls.isEmpty()) {
                        ...
                        if(i == 0) {
                            SingleTableSplitUtil.precheckSplitPk(conn, splitPkSql, fetchSize, table, userName,dataBaseType);
                        }
                    }
                } catch (ParserException e) {
